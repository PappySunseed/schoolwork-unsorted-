...
{	
	...
	while(cRow != 0 && cRow != 10 && cCol != 0 && cCol != 10 )
	{
		...
		if(Pnum > 1)
		{
			...
			display.push(cRow, cCol, mainStack.top->prevRow, mainStack.top->prevCol);
			vis[cRow][cCol] = true;
			...
			if(opt4 > opt3 && opt4 > opt2 && opt4 > opt1)
			{
				if(opt3 == 3)
					...
					display.push(cRow-1, cCol, cRow, cCol);
					cout << "pushed: " << cRow-1 << ","<< cCol << "       ";
				if(opt2 > 0)
					mainStack.push(cRow, cCol+1, cRow, cCol);
					display.push(cRow, cCol+1, cRow, cCol);
	...
	cout << "\nMouse is free at (" << cRow << ", " << cCol << ")\n\n";
	
	cout << "(right to left) Path, No backtracking:\n" <<mainStack << "\n\n";
	
	cout << "(right to left) Full Path:\n" << display << "\n";
