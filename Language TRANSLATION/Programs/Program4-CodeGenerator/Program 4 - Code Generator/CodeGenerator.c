...
int statement(char* buffer, char* errList, FILE* in, FILE* out, FILE* list)
{
	...
	switch(tokenSwitch)
	{
		case ID:
			...
			{
				printf("Token accepted: '%s'\n", buffer);
				strcpy(source.expression, buffer);
			}
...
int expression(char* buffer, char* errList, FILE* in, FILE* out, FILE* list)
{
	...
	strcpy(leftOperand.expression, exprBuff2.expression);
	...
	strcpy(leftOperand.expression, genInfix(leftOperand.expression, operator.expression, rightOperand.expression, out));
...
int factor(char* buffer, char* errList, FILE* in, FILE* out, FILE* list)
{
	...
	switch (swtch)
	{
		...
		case MINUSOP:
			...
			{
				strcpy(operator.expression, buffer);
				//strcat(writeBuff.expression, buffer);
				printf("Token accepted: '%s'\n", buffer);
			}
			factor(buffer, errList, in, out, list);
...
int start(char* buffer, FILE* out, char* outname)
{
	...
	{
		symbolTable[i][0] = NULL;	//set every entry in symbol table to null
	}
printf("\nproblem here?\n");
...
{
	//takes the two sides of the assignment as two assignment records 

	//generate(leftSide, "=", rightSide);	//creates a C assignment 
}
...
int processOP(char* buffer, FILE* out)
{
...
int finish(char* buffer, FILE* out)
{
	...
	fprintf(out, "\nreturn 0;\n}\n/*PROGRAM DID/DIDN'T COMPILE WITH A CERTAIN NUMBER OF ERRORS*/");		//adds closing data to C file as well as error information
...
int lookup(char* symbolTable[SYMBOL_TABLE_SIZE][MAX_STR_LENGTH], char* symbol) 
{
	...
	for (int i = 0; i < SYMBOL_TABLE_SIZE; i++) 
	{
		// Iterate through the strings in the current row
		for (int j = 0; j < MAX_STR_LENGTH; j++) 
		{
			// Check if the current string is not NULL and matches the search string
			if (symbolTable[i][j] != NULL && strcmp(symbolTable[i][j], symbol) == 0) 
			{
...
void getTemp(char* ID) 
{
	//if (tempNum >= MAX_TEMP_ARRAY_SIZE) {
	//	printf("Error: Maximum number of temporary strings exceeded.\n");
	//	return;
	//}

	//

	//// Create temporary ID string with current counter value appended to ID
	//snprintf(tempID, MAX_TEMP_LENGTH, "%s%d", ID, tempNum);

	//// Store the temporary ID in the array
	//tempStrings[tempNum] = tempID;
