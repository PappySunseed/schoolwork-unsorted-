...
getNumber PROC
    ...
    movzx ebx, byte ptr [buffer + ecx] ; Load character from wordbuffer into BL
    cmp ebx, 0       ; Check for end of string
    je convertfail  ; If end of string, exit loop
convert_loop:
    movzx ebx, byte ptr [buffer + ecx] ; Load character from wordbuffer into BL
    cmp ebx, 0       ; Check for end of string
...
PrintJobs_loop:
    cmp ecx, 0 ; check if we've printed all jobs
    je end_PrintJobs ; if so, end the procedure
    ...
    dec ecx ; decrement the loop counter
    jmp PrintJobs_loop ; repeat until all jobs have been printed
...
Process_LOAD PROC
  ...
  ;checks for job parameter 
call getNumber            ; Call the getNumber procedure
mov ecx, 0                ; Initialize index to 0

check_empty:
    mov bl, byte ptr [wordbuffer + ecx]  ; Load byte from wordbuffer
    cmp bl, 0                ; Check if end of string
    je read_input            ; If end of string, jump to read_input
    cmp bl, 'A'              ; Check if alphabetic character is uppercase
    jl not_uppercase         ; If not uppercase, jump to not_uppercase
    cmp bl, 'Z'
    jle havename             ; If uppercase, jump to havename
    cmp bl, 'a'              ; Check if alphabetic character is lowercase
    jl not_lowercase         ; If not lowercase, jump to not_lowercase
    cmp bl, 'z'
    jle havename             ; If lowercase, jump to havename
    inc ecx                  ; Move to next character
    jmp check_empty          ; Repeat until end of string
...
not_lowercase:
  ...
  mov edx, OFFSET wordbuffer ; point to the buffer
  mov ecx, SIZEOF wordbuffer ; specify max characters
  call ReadString ; read the input
...
Process_STEP PROC
    ...
    ; Decrement the runTime of the job by the specified amount
    sub jobRunTime[edi], ebx
...
findHighestPriority_loop:
    ...
    mov edx, jobPriority[edi] ; Get the priority of the current job
